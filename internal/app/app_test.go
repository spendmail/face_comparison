...
func TestApplication(t *testing.T) {
	...
	t.Run("unsupported file type", func(t *testing.T) {
		...
		require.True(t, len(multipleFaces) >= 2 && len(multipleFaces) <= 3, "multipleFaces != 2 or 3")
	})
	...
	t.Run("faces not found", func(t *testing.T) {
		...
		require.Equal(t, 1, len(facesNotFound), "facesNotFound != 0")
	})
	...
	t.Run("faces not found", func(t *testing.T) {
		...
		require.True(t, len(unmatched) >= 13 && len(unmatched) <= 14, "facesNotFound != 13 or 14")
	})
